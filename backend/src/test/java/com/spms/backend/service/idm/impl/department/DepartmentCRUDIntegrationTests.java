...
@Transactional
public class
DepartmentCRUDIntegrationTests {
    ...
    @Test
    void testCreateDepartment_validInput() {
        DepartmentModel model = new DepartmentModel();
        model.setName("New Department");
        model.setType(DepartmentType.LOCAL);
        model.setLevel(1);
        model.setParent(this.companyBusinessEnt.getId());
        
        DepartmentModel created = departmentService.createDepartment(model);
        assertNotNull(created.getId());
        assertEquals("New Department", created.getName());
    }

    @Test
    void testCreateDepartment_nullModel() {
    ...
    @Test
    void testCreateDepartment_nullType() {
        DepartmentModel model = new DepartmentModel();
        model.setName("Test");
        model.setType(null);
    ...
    void testGetDepartmentById_exists() {
        DepartmentModel result = departmentService.getDepartmentById(functionalDepartment.getId()).orElse(null);
        assertNotNull(result);
        assertEquals(functionalDepartment.getId(), result.getId());
    }

    @Test
    void testGetDepartmentById_notExists() {
        assertFalse(departmentService.getDepartmentById(-1L).isPresent());
    }
